...
void init_cn_crid_config(void) {
    ...
    g_config.latitude = 39.9042f;  // 北京坐标
    g_config.longitude = 116.4074f; // 北京坐标
    g_config.altitude_msl = 50.0f;
    g_config.altitude_agl = 45.0f;
    g_config.speed_horizontal = 8.0f;
    g_config.speed_vertical = 0.0f;
    g_config.heading = 45.0f;
    g_config.status = 2;  // Airborne (空中飞行)
    g_config.operator_lat = 39.9042f;
    g_config.operator_lon = 116.4074f;
    g_config.operator_alt = 50.0f;
...
void build_location_message_25bytes(uint8_t *message) {
    ...
    // 运行状态 (4 bits) + 高度类型 (1 bit) + 航迹角E/W标志 (1 bit) + 速度乘数 (1 bit) + 预留 (1 bit)
    uint8_t flags = (g_config.status << 4) | 0x01; // 状态 + Over Ground 高度类型
    message[1] = flags;
    ...
    if (ground_speed > 254) ground_speed = 254;
    message[3] = ground_speed;
    
    // 字节4: 垂直速度 - 符合试行标准表4
    int8_t vertical_speed = (int8_t)g_config.speed_vertical;
    ...
    uint16_t alt_baro_scaled = (uint16_t)(g_config.altitude_msl * 100); // 转换为厘米
    message[13] = alt_baro_scaled & 0xFF;
    message[14] = (alt_baro_scaled >> 8) & 0xFF;
    
    // 字节15-16: 几何高度 (小端序, cm) - 符合试行标准表4
    uint16_t alt_geo_scaled = (uint16_t)(g_config.altitude_msl * 100); // 转换为厘米
    message[15] = alt_geo_scaled & 0xFF;
    message[16] = (alt_geo_scaled >> 8) & 0xFF;
    
    // 字节17-18: 距地高度 (小端序, cm) - 符合试行标准表4
    uint16_t height_scaled = (uint16_t)(g_config.altitude_agl * 100); // 转换为厘米
...
void build_system_message_25bytes(uint8_t *message) {
    ...
    message[10] = 0x01; // 1 区域
    message[11] = 0x00;
    
    // 字节12: 运行区域半径 (m * 10) - 符合试行标准表6
    message[12] = 0x64; // 100m (1000 in *10 units, but only 1 byte, so 100m)
    
    // 字节13-14: 运行区域高度上限 (小端序, cm) - 符合试行标准表6
    uint16_t ceiling_scaled = (uint16_t)((g_config.altitude_msl + 100) * 100); // 100m above current
    message[13] = ceiling_scaled & 0xFF;
    message[14] = (ceiling_scaled >> 8) & 0xFF;
    
    // 字节15-16: 运行区域高度下限 (小端序, cm) - 符合试行标准表6
    uint16_t floor_scaled = (uint16_t)((g_config.altitude_msl - 50) * 100); // 50m below current
...
void build_beacon_frame(void) {
    ...
    *frame_len += ssid_len;
    ...
    uint8_t packed_msg[83]; // 3条消息 × 25字节 + 3字节头部 = 78字节
    uint8_t packed_len = 0;
    
    // Vendor Specific IE: OUI + Type + Packed Message
    frame[(*frame_len)++] = 0xDD; // Vendor Specific IE ID (221)
    frame[(*frame_len)++] = 5 + packed_len; // Length: OUI(3) + Type(1) + Counter(1) + Packed Message(packed_len)
...
static void send_beacon_task(void *pvParameter) {
    ...
    while (1) {
        // 模拟位置变化 (每10秒更新一次位置)
        if (xTaskGetTickCount() % (10000 / portTICK_PERIOD_MS) == 0) {
            demo_lat += 0.00001f; // 约1.1米
            demo_lon += 0.00001f; // 约0.9米
            demo_alt += 0.1f;    // 0.1m/s climb
            demo_speed_h = 5.0f + (esp_random() % 100) / 50.0f; // 5-7 m/s
