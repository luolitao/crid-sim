...
void init_cn_crid_config(void) {
    ...
    g_config.altitude_agl = 48.0f;
    g_config.speed_horizontal = 8.0f;
    g_config.speed_vertical = 0.0f;
    g_config.heading = 45.0f;
    g_config.status = 2;  // Airborne (空中飞行)
    g_config.operator_lat = 23.15f;
    g_config.operator_lon = 113.15f;
    g_config.operator_alt = 45.0f;
...
void build_location_message_25bytes(uint8_t *message) {
    ...
    uint16_t alt_baro_scaled = (uint16_t)(g_config.altitude_msl + 1000)*2; // 转换为厘米
...
void build_system_message_25bytes(uint8_t *message) {
    ...
    uint16_t ceiling_scaled = (uint16_t)((g_config.altitude_msl + 100) * 2); // 100m above current
    message[13] = ceiling_scaled & 0xFF;
    message[14] = (ceiling_scaled >> 8) & 0xFF;
    
    // 字节15-16: 运行区域高度下限 (小端序, cm) - 符合试行标准表6
    uint16_t floor_scaled = (uint16_t)((g_config.altitude_msl - 50) * 2); // 50m below current
...
static void send_beacon_task(void *pvParameter) {
    ...
    while (1) {
        ...
        if (xTaskGetTickCount() % (10000 / portTICK_PERIOD_MS) == 0) {
            demo_lat += 0.0001f; // 约1.1米
            demo_lon += 0.0001f; // 约0.9米
