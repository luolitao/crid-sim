// /Users/luolitao/esp32s3/crid-sim/main/crid-2.c
...
static void update_position_data(float lat, float lon, float alt_msl, float alt_agl, 
...
void init_cn_crid_config(void) {
    ...
    // 初始化巡游参数
    g_config.base_latitude = g_config.latitude;
    g_config.base_longitude = g_config.longitude;
    g_config.patrol_radius_lat = 0.00005f;  // 约5.5米
    g_config.patrol_radius_lon = 0.00004f;  // 约4.4米
    g_config.patrol_speed = 0.2f;
    g_config.time_counter = 0.0f;
...
static void send_beacon_task(void *pvParameter) {
    ...
    for (;;)
    {
        ...
        if ((xNow - xLastWake10s) >= xInterval10s)
        {
            ...
            // 模拟位置变化 (每10秒更新一次位置) - 在起点附近巡游
            ESP_LOGI(TAG, "================ 模拟位置变化 (每10秒更新一次位置) ================");
            
            // 使用时间作为参数来计算巡游位置，创建圆形路径
            g_config.time_counter += 1.0f; // 每10秒增加1
            
            // 使用正弦和余弦函数创建巡游路径
            // 计算新的位置（圆形路径）
            demo_lat = g_config.base_latitude + g_config.patrol_radius_lat * cosf(g_config.time_counter * g_config.patrol_speed);
            demo_lon = g_config.base_longitude + g_config.patrol_radius_lon * sinf(g_config.time_counter * g_config.patrol_speed);
            
            // 模拟高度变化（轻微波动）
            demo_alt = g_config.altitude_msl + 0.5f * sinf(g_config.time_counter * 0.1f);
            
            // 更新速度和航向以匹配巡游运动
            demo_speed_h = 1.5f + 0.5f * sinf(g_config.time_counter * 0.1f); // 在1.0-2.0 m/s之间变化
            demo_speed_v = 0.1f * cosf(g_config.time_counter * 0.1f);  // 轻微垂直速度变化
            
            // 计算航向角（基于运动方向）
            float dx = g_config.patrol_radius_lon * cosf(g_config.time_counter * g_config.patrol_speed) * g_config.patrol_speed;
            float dy = -g_config.patrol_radius_lat * sinf(g_config.time_counter * g_config.patrol_speed) * g_config.patrol_speed;
